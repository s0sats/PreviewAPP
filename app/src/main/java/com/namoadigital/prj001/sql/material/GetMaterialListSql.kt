...
class GetMaterialListSql(
    ...
    private fun getAffinityCounterQuery(searchWords: List<String>): String {
        val productDescId = "p.product_desc || ' (' || p.product_id || ') '"
        val productDesc = "p.product_desc"
        return """ CASE WHEN p.product_id != p.product_code 
                        THEN case when lower($productDescId) GLOB lower('*$s_filter*')
                                  then 999
                                  else ${getWordsCriteria(searchWords, productDescId)}
                             end      
                        ELSE case when lower($productDesc) GLOB lower('*${s_filter}*')
                                  then 999
                                  else ${getWordsCriteria(searchWords, productDesc)}
                             end    
                   END SCORE, 
        """.trimIndent()
    }

    private fun getWordsCriteria(searchWords: List<String>, criteria:String): String{
        return searchWords
            .asSequence()
            .map { it.trim() }
