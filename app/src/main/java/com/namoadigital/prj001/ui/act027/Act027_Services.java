...
//        Log.d("Permissao","onResume");
    ...
    private boolean checkIfNeedSync() {
        return mSm_so.getUpdate_required() == 1 || isSoWithinTokenFile() || sm_soDao.hasSyncRequired(mSm_so.getCustomer_code(), mSm_so.getSo_prefix(), mSm_so.getSo_code());
    }
...
//                    Log.d("SHORTCUT_STOP", "Exec_tmp e task_tmp não encontrado.");
    ...
    private SM_SO_Service_Exec_Task createTask(SM_SO_Service_Exec serviceExec, String task_perc) {
        ...
        newTask.setSite_code(Integer.valueOf(ToolBox_Con.getPreference_Site_Code(context)));
        newTask.setZone_code(ToolBox_Con.getPreference_Zone_Code(context));
