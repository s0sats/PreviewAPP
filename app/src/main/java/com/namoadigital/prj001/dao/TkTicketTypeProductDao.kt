...
class TkTicketTypeProductDao (
    ...
    private val toTkTicketTypeProductMapper: Mapper<Cursor, TkTicketTypeProduct>
    private val toContentValuesMapper: Mapper<TkTicketTypeProduct, ContentValues>

    init {
        toTkTicketTypeProductMapper = CursorToTkTicketTypeProductMapper()
        toContentValuesMapper = TkTicketTypeProductToContentValuesMapper()
    }

    override fun addUpdate(tkTicketTypeProduct: TkTicketTypeProduct?): DaoObjReturn {
    ...
    private fun getWherePkClause(tkTicketTypeProduct: TkTicketTypeProduct?): StringBuilder {
    ...
    override fun addUpdate(tkTicketChaces: MutableList<TkTicketTypeProduct>?, status: Boolean): DaoObjReturn {
        var daoObjReturn = DaoObjReturn()
        var addUpdateRet: Long = 0
        var curAction = DaoObjReturn.INSERT_OR_UPDATE
        //
    ...
    override fun getByString(sQuery: String): TkTicketTypeProduct? {
        var tkTicketTypeProduct: TkTicketTypeProduct? = null
        openDB()
        try {
            val cursor = db.rawQuery(sQuery, null)
            while (cursor.moveToNext()) {
                tkTicketTypeProduct = toTkTicketTypeProductMapper.map(cursor)
            }
            //
            cursor.close()
        } catch (e: java.lang.Exception) {
    ...
    override fun query(sQuery: String?): MutableList<TkTicketTypeProduct> {
        val tkTicketTypeProducts = mutableListOf<TkTicketTypeProduct>()
        openDB()
        try {
            val cursor = db.rawQuery(sQuery, null)
            while (cursor.moveToNext()) {
                val uAux = toTkTicketTypeProductMapper.map(cursor)
    ...
    private inner class TkTicketTypeProductToContentValuesMapper : Mapper<TkTicketTypeProduct, ContentValues> {
        override fun map(tkTicketTypeProduct: TkTicketTypeProduct?): ContentValues {
    ...
    private inner class CursorToTkTicketTypeProductMapper : Mapper<Cursor, TkTicketTypeProduct> {
        override fun map(cursor: Cursor?): TkTicketTypeProduct? {
            cursor?.let {
                with(cursor) {
                    return TkTicketTypeProduct(
