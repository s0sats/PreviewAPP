...
class FSTripDao @Inject constructor(
    ...
    private fun handlePdfChanges(
        ...
        fsTrip.destinations?.forEach { remoteDestination ->
            remoteDestination.actions?.forEach { remoteAction ->
                val file = remoteAction.actPDFName?.let { remoteName -> File(ConstantBaseApp.CACHE_PATH,ConstantBaseApp.N_FORM_PDF_PREFIX + remoteName) }
                file?.let { f ->
                    ...
                    if (f.exists()
                        ...
                        remoteAction.actPDFLocal = ConstantBaseApp.N_FORM_PDF_PREFIX + remoteAction.actPDFName
                    }else{
                        ...
                        action?.let {action ->
                            action.actPDFLocal?.let{localFile ->
                                Log.d("TRIP_FILE", "remoteAction.actPDFName != action.actPDFName " + (remoteAction.actPDFName != action.actPDFName))
    ...
    fun updateFleet(
        ...
        val photoName = if (target == TripTarget.START){
            if(!photoPath.isNullOrBlank()){
                ...
                "$FLEET_START_PHOTO_NAME = null"
            }
        } else{
            if(!photoPath.isNullOrBlank()){
                "$FLEET_END_PHOTO_NAME = '$photoPath'"
            }else{
                "$FLEET_END_PHOTO_NAME = null"
            }
        }
        //
        val odometerQuery =
            if (target == TripTarget.START) {
                ...
                }?: "$FLEET_START_ODOMETER = null"
            } else{
                odometer?.let{
                    "$FLEET_END_ODOMETER = $odometer"
                }?: "$FLEET_END_ODOMETER = null"
            }
    ...
    fun addTripDestination(
        ...
        return trip?.let {
            ...
            try {
                db.beginTransaction()
                updateStatus(it.tripPrefix, it.tripCode, scn, tripStatus, db)
