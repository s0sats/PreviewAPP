...
import com.namoadigital.prj001.sql.IO_Move_Reason_Sql_Truncate;
...
public class WS_Sync extends IntentService {
    ...
    private void processWS_Sync(String session_app, ArrayList<String> dataPackageType, int jump_validation, int jump_od, Long product_code, boolean loginProcess, int connection_timeout) throws Exception {
         ...
         * Aqui no sincronismo caso data seja invalida, além de abortar o processamento,
...
//            }
            ...
            //
            File[] file_io_move_reason = ToolBox_Inf.getListOfFiles_v2("io_move_reason-");

            for (File _file : file_io_move_reason) {

                ArrayList<IO_Move_Reason> io_move_reasons = gson.fromJson(
                        ToolBox.jsonFromOracle(
                                ToolBox_Inf.getContents(_file)
                        ),
                        new TypeToken<ArrayList<IO_Move_Reason>>() {
                        }.getType()
                );
