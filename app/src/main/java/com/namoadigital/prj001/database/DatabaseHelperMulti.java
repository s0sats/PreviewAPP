...
public class DatabaseHelperMulti extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        try {
            ...
            script.append("create table if not exists [md_product_serials]([customer_code] int not null,[product_code] int not null,[serial_code] int not null,[serial_id] text not null collate nocase,[site_code] int,[zone_code] int,[local_code] int ,[site_code_owner] int,[brand_code] int,[model_code] int,[color_code] int,[segment_code] int,[category_price_code] int,    [add_inf1] text COLLATE NOCASE, [add_inf2] text COLLATE NOCASE,[add_inf3] text COLLATE NOCASE, [update_required] int not null default 0,[only_position] int,constraint [pk_md_product_serials] primary key([customer_code],[product_code],[serial_code]));");
            script.append("CREATE TABLE IF NOT EXISTS [md_product_serial_trackings]([customer_code] int not null,[product_code] int not null,[serial_code] int not null,[serial_tmp] int,[tracking] int not null,constraint [pk_md_product_serial_trackings] primary key([customer_code],[product_code],[serial_code],[tracking]));");
