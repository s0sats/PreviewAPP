...
public class DatabaseHelperMulti extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        try {
            ...
            script.append("CREATE TABLE if not exists [sm_so_service_exec_tasks] ( [customer_code] int NOT NULL, [so_prefix] int NOT NULL, [so_code] int NOT NULL, [price_list_code] int NOT NULL, [pack_code] int NOT NULL, [pack_seq] int NOT NULL, [category_price_code] int NOT NULL, [service_code] int NOT NULL, [service_seq] int NOT NULL, [exec_code] int , [task_code] int, [exec_tmp] int NOT NULL, [task_tmp] int NOT NULL,[task_seq_oper] int NOT NULL, [task_user] int NOT NULL, [task_user_nick] text NOT NULL, [start_date] text NOT NULL, [end_date] text, [exec_time] int, [exec_time_format] text, [task_perc] int NOT NULL, [qty_people] int NOT NULL, [status] text NOT NULL, [site_code] int NOT NULL, [site_id] text NOT NULL, [site_desc] text NOT NULL, [zone_code] int NOT NULL, [zone_id] text NOT NULL, [zone_desc] text NOT NULL, [local_code] int NOT NULL, [local_id] text NOT NULL, [comments] text, PRIMARY KEY([customer_code],[so_prefix],[so_code],[price_list_code],[pack_code],[pack_seq],[category_price_code],[service_code],[service_seq],[exec_tmp],[task_tmp]));");
            script.append("CREATE TABLE if not exists [sm_so_service_exec_task_files]( [customer_code] int NOT NULL, [so_prefix] int NOT NULL, [so_code] int NOT NULL, [price_list_code] int NOT NULL, [pack_code] int NOT NULL, [pack_seq] int NOT NULL, [category_price_code] int NOT NULL, [service_code] int NOT NULL, [service_seq] int NOT NULL, [exec_code] int, [task_code] int, [file_code] int, [exec_tmp] int NOT NULL, [task_tmp] int NOT NULL,[file_tmp] int NOT NULL, [file_name] text NOT NULL, [file_url] text NOT NULL, PRIMARY KEY([customer_code] ,[so_prefix] ,[so_code] ,[price_list_code] ,[pack_code] ,[pack_seq] ,[category_price_code] ,[service_code] ,[service_seq] ,[exec_tmp] ,[task_tmp] ,[file_tmp]));");
