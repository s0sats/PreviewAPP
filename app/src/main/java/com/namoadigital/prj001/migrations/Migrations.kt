...
val MigrationV1 = object : MigrationSQLite(1, 2){
    override fun migrate(db: SQLiteDatabase) {
        ...
        db.execSQL(
            """
                ALTER TABLE ${TK_TicketDao.TABLE}
                        ADD COLUMN ${TK_TicketDao.OPEN_ZONE_CODE} int;
            """.trimIndent()
        )
        //
        db.execSQL(
            """
                ALTER TABLE ${TK_TicketDao.TABLE}
                        ADD COLUMN ${TK_TicketDao.OPEN_ZONE_ID} text COLLATE nocase;
            """.trimIndent()
        )
        //
        db.execSQL(
            """
                ALTER TABLE ${TK_TicketDao.TABLE}
                        ADD COLUMN ${TK_TicketDao.OPEN_ZONE_DESC} text COLLATE nocase;
            """.trimIndent()
        )
        //
        db.execSQL(
